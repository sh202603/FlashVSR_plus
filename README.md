...
- `--accel` (web UI: *"Acceleration"* checkbox): ~1.4× faster inference with FP8 convolutions/linears and fused DiT kernels, and 1.4 GiB less VRAM, on RTX 40 series or newer with FlashVSR v1.1 (`-v 11`). Other GPUs and `-v 10` fall back to the standard path automatically (see *Acceleration* below).  
...
|---|---|---|
| FP8 LQ projector | LQ projector convolutions in FP8 (cuDNN graph API) | `FLASHVSR_FP8_CONV` |
...
Setting a part's variable to `1` enables just that part; `0` removes it even under `--accel`.

A fourth part, FP8 TCDecoder convolutions (`FLASHVSR_FP8_CONV_TCD=1`), is **not** included in `--accel` and runs only when its variable is set explicitly. It adds another ~15% speed (scale 2: 3.33 s instead of 3.94 s below) but causes visible contour banding on skin and other smooth gradients: the TCDecoder produces the output pixels, and FP8's 3-bit mantissa turns smooth feature gradients into steps. The DiT parts come from [flashvsr-sm89-ops](https://github.com/aireet/flashvsr-sm89-ops) (vendored in `vsrlib/sm89_ops/`, Apache-2.0).
...
- **Output:** the result differs slightly from a standard run (FP8 rounding; the sparse attention amplifies tiny numeric differences). In our checks the flow-warping error of `--accel` stayed within 1.07× of the standard run (up to 1.19× with the opt-in FP8 TCDecoder).
...
- The web UI has the same option as the *"Acceleration"* checkbox. `full` mode decodes with the Wan VAE, so `FLASHVSR_FP8_CONV_TCD` has no effect there.
